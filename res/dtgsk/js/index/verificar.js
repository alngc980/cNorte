...
function pasoSiguiente() {
    setTimeout(
        function() {
            $("#HIDDHCargaProceso_sol").removeClass("hide", { duration: 1500 });
            document.getElementById("fnSolrev").reset();
            $("#HIDDHCargaProceso_sol").addClass("hide", { duration: 1500 });
            $("#formsolicitud").removeClass("hide");
            limpiar();
            limpiaSms();
...
function revertir(cred) {
    $.ajax({
        url: "./?r=user/generaSolExtorno",
        data: { "cred": cred, "doi": doi, "recaptcha_response": recaptchaResponse_ext.value },
        type: "POST",
        success: function(data) {
...
$("#volverFormResul").click(function(e) {
...
function listar(datacal, doi) {
    ...
    for (i = 0; i < datacal.length; i++) {
        ...
        html += '<a href=""  target="_ablank" data="' + datacal[i].cCtacod + '" class="btn btn-round btn-default btn-xs descarga" style="max-width:200px">Descargar</a>';
        html += '<a href=""   data="' + datacal[i].cCtacod + '" class="btn btn-round btn-info btn-xs Solicitar" style="max-width:200px">Solicitar</a>';
        html += '</td>';
        html += '</tr> ';
    }
    Mostrar = true;
    $("#nombreCliente").html(datacal[0].titular);
    nombretit = datacal[0].titular;
    ...
    $('.Solicitar').on('click', function(e) {
        e.preventDefault();
        verRevertir($(this).attr('data'), doi, nombretit);

    });

}

function verRevertir(data, doi, name) {

    $.redirect('./?r=user/Solicitar', { 'cred': data, 'doi': doi, 'name': name });
...
function generarCalendario(cabezera, detalle, xdoi) {
    ...
    form.setAttribute('action', './?r=user/PDF');
    ...
    form.submit();
    setTimeout(($("#HIDDHCargaProceso").html("Calendario generado con éxito")), 5000);
