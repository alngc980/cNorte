...
-- Tiempo de generación: 05-02-2022 a las 23:13:12
...
-- Procedimientos
--
CREATE DEFINER=`gqouvkku`@`localhost` PROCEDURE `spt_ins_documento` (IN `nIdUserRegistro` INT, IN `nIdPrograma` INT, IN `nIdTarea` INT, IN `nIdTipoDocumento` INT, IN `nIdEmpresa` INT, IN `nIdUUNN` INT, IN `nIdCargo` INT, IN `nIdResponsable` INT, IN `nIdSupervision` INT, IN `cActividad` VARCHAR(500), IN `cObra` VARCHAR(500), IN `cLugar` VARCHAR(500), IN `cArea` VARCHAR(220), IN `cFecha` DATE, IN `nActa` INT, IN `cESAS` VARCHAR(220))  BEGIN
    SET @CANT := (SELECT MAX(I.`nIdAux`) FROM informe I WHERE nIdUserRegistro NOT IN (17))+1;
    /*IF (@CANT=1)
    THEN
    	SET @CANT := 400;
    END IF;*/
    ...
    IF (nIdUserRegistro = 17)
    THEN
		SET @CANT :=  (SELECT MAX(I.`nIdAux`) FROM informe I WHERE nIdUserRegistro = 17 )+1;
    END IF;
    
    /*nIdUserRegistro*/
    
	INSERT INTO informe (
	nIdAux,nIdUserRegistro,nIdPrograma,nIdTarea,nIdTipoDocumento,nIdEmpresa,nIdUUNN,nIdCargo,nIdResponsable,
	nIdSupervision,cActividad,cObra,cLugar,cArea,nActa,cESAS,nEstado,cFecha	
	) 
	VALUE (
	@CANT,nIdUserRegistro,nIdPrograma,nIdTarea,nIdTipoDocumento,nIdEmpresa,nIdUUNN,nIdCargo,nIdResponsable,
	nIdSupervision,cActividad,cObra,cLugar,cArea,nActa,cESAS,1,cFecha
	);
	 SELECT LAST_INSERT_ID() IDKEY;
END$$

CREATE DEFINER=`gqouvkku`@`localhost` PROCEDURE `spt_ins_documento_detalle` (IN `nIdInforme` INT, IN `nIdCumplimiento` INT, IN `nValor` INT, IN `nIdInc` INT, IN `cDescripcion` VARCHAR(5000))  BEGIN
	
	INSERT INTO inf_cumplimiento_des (
	nIdInforme,nIdCumplimiento,nValor,nValorInc, cDescripcion	
	) 
	VALUE (
	nIdInforme,nIdCumplimiento,nValor,nIdInc,cDescripcion);
END$$

CREATE DEFINER=`gqouvkku`@`localhost` PROCEDURE `spt_ins_user` (`cNombres` VARCHAR(250), `cApellidos` VARCHAR(250), `cDoi` VARCHAR(8), `cPassword` VARCHAR(250))  BEGIN
	
	INSERT INTO persona (cNombres ,cApellidos,cDoi,cPassword) 
	VALUE (cNombres,cApellidos, cDoi, cPassword);
	
	INSERT INTO `responsable`(`nIdPersona`,`nIdEmpresa`,`nFlag`,`dRegistro`)
	VALUES (LAST_INSERT_ID(),1,0,DEFAULT);
	
END$$

CREATE DEFINER=`gqouvkku`@`localhost` PROCEDURE `spt_sel_all_User` (`x_CodUser` VARCHAR(10))  BEGIN
	SET @bAdmin := (SELECT bAdmin FROM persona WHERE nId=x_CodUser);
	IF @bAdmin < 1 THEN
		SELECT * FROM persona WHERE nEstado IN (0,1) AND nId = x_CodUser;
	ELSE
		SELECT * FROM persona WHERE nEstado IN (0,1) ORDER BY `dRegistro` DESC;
	END IF;
END$$

CREATE DEFINER=`gqouvkku`@`localhost` PROCEDURE `spt_sel_inspeciones` (IN `dAnio` INT)  BEGIN
SELECT TAB.CNOMBRE,
TAB.ENE,TAB.FEB,TAB.MAR,TAB.ABR,
TAB.MAY,TAB.JUN,TAB.JUL,TAB.AGO,
TAB.SET,TAB.OCT,TAB.NOV,TAB.DIC,
(
TAB.ENE+TAB.FEB+TAB.MAR+TAB.ABR+
TAB.MAY+TAB.JUN+TAB.JUL+TAB.AGO+
TAB.SET+TAB.OCT+TAB.NOV+TAB.DIC)TOT
FROM(
SELECT
I.nIdEmpresa, e.`cNombre`,
COUNT(
        CASE 
            WHEN MONTH(I.dRegistro)=1 
            THEN 1 
            ELSE NULL 
        END
    ) AS 'ENE',
...
WHERE
  YEAR(I.dRegistro)=dAnio
GROUP BY
 I.`nIdEmpresa`)TAB;
 END$$

CREATE DEFINER=`gqouvkku`@`localhost` PROCEDURE `spt_sel_obtienedocumento` (IN `X_User` INT)  BEGIN		
	SET @row_number = 0; 

	IF(X_User IN (17,3))
	THEN
		SELECT 	(@row_number:=@row_number + 1) nFila,
		T.`cNombre` cTipo,
		I.`nId` nId,
		P.`cNombres` cUser,
...
order by I.nIdAux desc;
	...
	ELSE
		...
		JOIN tipo_documento T ON T.`nId` = I.`nIdTipoDocumento`
		WHERE I.nFlag IN (0)
		AND I.nIdUserRegistro = X_User
order by I.nIdAux desc;

	END IF;
END$$

CREATE DEFINER=`gqouvkku`@`localhost` PROCEDURE `spt_sel_VerificaUltimoAdmin` (`x_nId` INT)  BEGIN
	IF EXISTS (SELECT * FROM persona WHERE bAdmin=0 AND nId=x_nId)	/*SI EXISTE, ESTAN HABILITANDO ADMINISTRACION*/
	THEN
		SELECT 0 Cant;
	ELSE
		/*SI ESTAN DESHABILITANDO ADMINISTRACION*/
		SET @cantidad := (SELECT COUNT(*) FROM persona WHERE bAdmin=1);
		IF (@cantidad>1)		/*SI HAY MAS DE 1 ADMINISTRADOR*/
		THEN
			SELECT 0 Cant;
		ELSE
			SELECT 1 Cant;		/*es el ultimo adm*/
		END IF;
	END IF;
END$$

CREATE DEFINER=`gqouvkku`@`localhost` PROCEDURE `stp_ins_responsable` (IN `PcDoi` VARCHAR(500), IN `PNombres` VARCHAR(500), IN `PApellidos` VARCHAR(500), IN `PnIdEmpresa` INT)  BEGIN
    
	INSERT INTO persona (cDoi, cNombres, cApellidos)
	VALUES (PcDoi,PNombres,PApellidos);
	
	SET @nIdXX = (SELECT LAST_INSERT_ID());
	
	INSERT INTO `responsable`(`nIdPersona`, `nIdEmpresa`, `nFlag`)
	VALUES (@nIdXX, PnIdEmpresa, 0);
	
	SELECT @nIdXX nIdPersona;
	
    END$$

CREATE DEFINER=`gqouvkku`@`localhost` PROCEDURE `stp_sel_resumen_informe` (IN `x_nIdInforme` INT)  BEGIN
	SELECT 'TOTAL DE CUMPLIMIENTO (NC)' DET, 
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 1) EPP,
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 2) SE, 
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 3) UM,
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 4) DOC, 
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 5) CP,
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 6) CS, 
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d INNER JOIN `cumplimiento` c ON d.`nIdCumplimiento` = c.`nId` AND c.`cCodigo` <> ''
			WHERE `nIdInforme` = x_nIdInforme AND nValor = 1) TOTAL,
		
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d INNER JOIN `cumplimiento` c ON d.`nIdCumplimiento` = c.`nId` AND c.`cCodigo` <> ''
			WHERE `nIdInforme` = x_nIdInforme AND nValor = 1)/
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d INNER JOIN `cumplimiento` c ON d.`nIdCumplimiento` = c.`nId` AND c.`cCodigo` <> ''
			WHERE `nIdInforme` = x_nIdInforme AND nValor = 1)+
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d INNER JOIN `cumplimiento` c ON d.`nIdCumplimiento` = c.`nId` AND c.`cCodigo` <> ''
			WHERE `nIdInforme` = x_nIdInforme AND nValor = 2))
		)*100.00 PORCENTAJES
	UNION
	SELECT 'TOTAL DE INCUMPLIMIENTO (NI)'  DET, 
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 1) EPP,
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 2) SE, 
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 3) UM,
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 4) DOC, 
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 5) CP,
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 6) CS, 
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d INNER JOIN `cumplimiento` c ON d.`nIdCumplimiento` = c.`nId` AND c.`cCodigo` <> ''
			WHERE `nIdInforme` = x_nIdInforme AND nValor = 2) TOTAL,
	...
	UNION
	SELECT 'NIVEL DE CUMPLIMIENTO' DET,
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 1)/
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 1)+
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 1))*100.00) EPP,
		
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 2)/
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 2)+
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 2))*100.00) SE,
		
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 3)/
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 3)+
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 3))*100.00) UM,
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 4)/
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 4)+
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 4))*100.00) DOC,
		
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 5)/
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 5)+
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 5))*100.00) CP,
		
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 6)/
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 6)+
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 6))*100.00) CS,
		
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d INNER JOIN `cumplimiento` c ON d.`nIdCumplimiento` = c.`nId` AND c.`cCodigo` <> '' WHERE `nIdInforme` = x_nIdInforme AND nValor = 1)/
		(
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d INNER JOIN `cumplimiento` c ON d.`nIdCumplimiento` = c.`nId` AND c.`cCodigo` <> '' WHERE `nIdInforme` = x_nIdInforme AND nValor = 1)+
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` d INNER JOIN `cumplimiento` c ON d.`nIdCumplimiento` = c.`nId` AND c.`cCodigo` <> '' WHERE `nIdInforme` = x_nIdInforme AND nValor = 2)
		))*100.00 TOTAL,
		100 PORCENTAJES
	;
END$$

CREATE DEFINER=`gqouvkku`@`localhost` PROCEDURE `stp_sel_resumen_informe2` (IN `x_nIdInforme` INT)  BEGIN
	SELECT 'FINAL' DET,
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 1)/
		((SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 1 AND `nIdCumplimiento` = 1)+
		(SELECT COUNT(`nValor`) CANT FROM `inf_cumplimiento_des` WHERE `nIdInforme` = x_nIdInforme AND nValor = 2 AND `nIdCumplimiento` = 1))*100.00) EPP,
