...
public static function getDatosEmpleados($idEmpresa){
	$sql = "SELECT r.nId,p.`cNombres`,p.`cApellidos` FROM responsable r JOIN persona p ON p.`nId` = r.`nIdPersona` JOIN empresa e ON e.`nId`=r.`nIdEmpresa` WHERE e.nId = ".$idEmpresa." /*WHERE e.`nTipo`=1*/";
