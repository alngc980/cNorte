...
class informeData {
	...
	public function add(){
		...
		$sql .="(\"$this->nIdUserRegistro\",\"$this->nIdPrograma\",";
		$sql .="\"$this->nIdTarea\",\"$this->nIdTipoDocumento\",\"$this->nIdEmpresa\",";
		$sql .="\"$this->nIdUUNN\",\"$this->nIdCargo\",\"$this->nIdResponsable\",";
		$sql .="\"$this->nIdSupervision\",\"$this->cActividad\",\"$this->cObra\",";
		$sql .="\"$this->cLugar\",\"$this->cArea\",\"$this->cFecha\",\"$this->nActa\",\"$this->cESAS\")";
	...
	public function getInformeById_reporte()
	{
		$sql = "select * from ".self::$tablename."  where  nId=$this->nId and nIdTarea !=3";
		$query = Executor::doit($sql);
		return Model::one($query[0],new informeData());
	}
	public function getInformeById_reporte_charlas()
	{
		$sql = "select p.cNombres,p.cApellidos,  c.* from ".self::$tablename."  c
		JOIN responsable r on r.nId= c.nIdResponsable
		JOIN persona p on p.nId=r.nIdPersona 
		where  c.nId=$this->nId and nIdTarea =3";
		$query = Executor::doit($sql);
		return Model::one($query[0],new informeData());
	}
